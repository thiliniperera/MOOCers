...
plot(newdata_withQuizBrainGrowth$anon_screen_name.,newdata_withQuizBrainGrowth$ScaledDuration, xlab="Student", ylab="Duration",main="Students Vs Duration", las=1)
...
nn$result.matrix
par(mfrow=c(2,2))
gwplot(nn,selected.covariate="train$q1_grade", min=-2.5, max=5)
...
test_dup <-test
test_dup$prediction <- prediction

# Write test results CSV in R
write.csv(test_dup, file = "//home/chamil/Documents/work/MOOCers-master/AssessmentDropout/MyData_test_with16.csv")

#read results
written_results = read.csv("/home/chamil/Documents/work/MOOCers-master/AssessmentDropout/MyData_test_with16.csv")
...
#Add prediction column to result set
prediction <- c(results_grade_for_grandset$net.result)
grandDataset_with_prediction <-grandDataset
grandDataset_with_prediction$prediction <- prediction

# Write test results CSV in R
write.csv(grandDataset_with_prediction, file = "//home/chamil/Documents/work/MOOCers-master/AssessmentDropout/grandDataset_with_prediction_16.csv")

#read results
written_results_grand = read.csv("/home/chamil/Documents/work/MOOCers-master/AssessmentDropout/grandDataset_with_prediction_16.csv.csv")

#Calculate Error
predict_nn <- results_grade_for_grandset$net.result*(max(grandDataset$grade.)-min(grandDataset$grade.))+min(grandDataset$grade.)
predict_nn
test_results <- grandDataset$grade.*(max(grandDataset$grade.)-min(grandDataset$grade.))+min(grandDataset$grade.)
test_results
